...
bot.on("message", (msg: telegramBot.Message) => {
  ...
  if (greetings.includes(msg.text)) {
    if (msg.from?.id === admin) {
      ...
      bot.sendMessage(
        chatId,
        `Hello ${msg.chat.first_name} @${msg.chat.username}`
      );
  }
});
...
bot.onText(
  /\/exec (.+)/,
  (msg: telegramBot.Message, match: RegExpExecArray | null) => {
    const LogID: number = process.env.LOGS
      ? Number(process.env.LOGS)
...
bot.on("message", async (msg) => {
  ...
  bot.sendMessage(msg.chat.id, geminiResponse);
});
...
bot.onText(/^\/unsubscribe$/, (msg: telegramBot.Message) => {
  ...
  User.deleteOne({ id: msg.chat.id }, (err) => {
    if (err) {
      bot.sendMessage(
        Number(process.env.ADMIN),
        "Error deleting user from database " + err.message
      );
    }
  });
});
...
bot.onText(
  ...
  (msg: telegramBot.Message, match: RegExpExecArray | null) => {
    if (!isAuthorized(msg)) {
      bot.sendMessage(
        msg.chat.id,
        "Due to openAI's limitations, this command is only available to authorized users"
      );
      return;
    }
    if (!match) {
